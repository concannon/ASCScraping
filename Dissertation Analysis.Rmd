...
## Term Graph
This graph also shows the correlation between terms mentioned 15+ times.  The strongest relationships seem to be between risk and factors, and sex and offender.  The upper-left also shows an interesting cluster of policing and victimization terms.  I am not sure of the layout of this graph, so maybe it is nothing.

The reduced plot of terms mentioned 20 times or more is easier to read.  Clear links between policing, community, and offenders.  Some of the terms are not properly 'stemming' - policing and police, offender and offending.
```{r,echo=T,warning=F}
...
freq.terms <- findFreqTerms(term.matrix,lowfreq=20)
plot(term.matrix,term=freq.terms,corThreshold = .05,weighting=T)
```


## Tidy Text
This a package from David Robinson and Julia Silge.  Some examples straight from their vignettes.  Risk and offender have the most negative sentiment, while work, support, and intimate are mostly positive.
```{r}
